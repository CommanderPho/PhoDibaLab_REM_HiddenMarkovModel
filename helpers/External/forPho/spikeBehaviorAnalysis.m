function [newSpikes, units, spikesTable]  = spikeBehaviorAnalysis(spikes, laps, ripples, speed, qual2consider, fileinfo)
...
%%% 'interneuron', and 'all'
    ...
    %% Build Spikes table:
    spikesTable = struct2table(spikes);
    % Count the spikes for each unit
    spikesTable.num_spikes = cellfun((@(timestamps) length(timestamps)), spikesTable.time);
    % Get the classification type (Pyramidal, interneuron, etc) for each unit:
    [spikesTable.speculated_unit_type, spikesTable.speculated_unit_contamination_level, ~] = fnUnitQualityToCellType(spikesTable.quality);

    spikesTable.stability_count = sum(spikesTable.isStable, 2);
    spikesTable.isAlwaysStable = (spikesTable.stability_count == 3);
